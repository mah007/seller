...
class AutoHistoryWorker(threading.Thread):
	...
	def run(self):
		...
		for sku in skus:
			...
			autoPriceManager.insertHistory(sku, enemies)
	...
	def getEnemies(self, user, sku):
		...
		topEnemyJson =  topEnemyName[0].replace(' ','') + ' - ' + str(int(topEnemyPrice[0].replace('VND', '').replace('.', '').replace(',', '')))
		...
		for index, enemy in enumerate(enemies):
			otherEnemyJson = enemy.replace(' ','') + ' - ' + str(int(enemyPrices[index].replace('VND', '').replace('.', '')))
			enemiesJson.append({
				otherEnemyJson
				})
